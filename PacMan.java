...
public class PacMan extends JPanel implements ActionListener, KeyListener {
    ...
    private HashSet<Block> cherries;
    ...
    boolean ghostsVulnerable = false;
        ...
        for (int r = 0; r < rowCount; r++) {
            for (int c = 0; c < columnCount; c++) {
                ...
                if (tileMapChar == 'X') {
                    ...
                    foods.add(food);
                    // Randomly place cherries
                    if (random.nextInt(20) < 1) { // 5% chance to place a cherry
    ...
    public void move() {
        ...
        for (Block ghost : ghosts) {
            if (collision(ghost, pacman)) {
                if (ghostsVulnerable) {
                    ghosts.remove(ghost);
                    score += 200;
                    resetPositions(); // Restart game when PacMan hits a vulnerable ghost
                    return;
                } else {
                    lives -= 1;
                    if (lives == 0) {
        ...
        for (Block cherry : cherries) {
            if (collision(pacman, cherry)) {
                ...
                vulnerableTime = 200; // duration of vulnerability
            }
        }
